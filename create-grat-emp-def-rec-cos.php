...
<head>
    ...
    <title>Create Gratuity COS</title>
        ...
        for($i = 0; $i < sizeof($_POST['employee']); $i++) {
            ...
            if($_POST['employee'][$i] != '') {
                $query = mysqli_query($db, "INSERT INTO gratuity_tbl(payroll_id,name_id,officecode,employmenttype,date_created) 
                                            VALUES('$payroll_id','$employee','$officecode','$type','$date')");
            }
           
        
        }
        echo '<script>swal("Awesome!", "Payroll Successfully Saved!", "success");</script>'; 
       // echo '<script>document.location = "grat-records-cos.php?payroll_id='.$payroll_id.'"</script>';
    }else{
...
// get all emplyee add to an ARRAY
     $query = mysqli_query($db, "SELECT * FROM employment_info WHERE isactive='yes' AND employmenttype = 'CS' AND atmno != '0' AND claimed='yes' ");
 ...
 if( count($employeeall) == count(array_count_values($employeeall)) ) {
        ...
        echo '<script>swal("Awesome!", "Payroll Successfully Saved!", "success");</script>'; 
       // echo '<script>document.location = "grat-records-cos"</script>';
    }
                ...
                echo '<script>swal("Information!", "There is no COS employee in your chosen office", "info");</script>';
...
<section id="multiple-column-form">
        <div class="row match-height" style="margin:30px 30px 30px 30px;">
            <div class="col-12">
                <div class="card" >
                    <div class="card-header">
                        <h4 class="card-title text-center">Create COS Defiency</h4>
                    </div>
            ...
            <form action="create-gratuity-cos.php" method="post">
...
xhr.open("POST",`load-emp-def-cos.php`, true);
