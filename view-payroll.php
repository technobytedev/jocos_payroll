    ...
    if(count($_POST['employee'])==count(array_count_values($_POST['employee']))){
        ...
        for($i = 0; $i < sizeof($_POST['employee']); $i++) {
            ...
            if($_POST['employee'][$i] != '') {
                ...
                //check duplicates
                if($numrow == 0) {
                $query = mysqli_query($db, "INSERT INTO payroll_tbl(payroll_id,name_id,working_days,charge,mode_of_payment,type,position_code,salary_rate,officecode,date_from,date_to,date_created,net_pay,tax) 
                                            VALUES('$payroll_id','$emp_id[0]','$days','$charge','$payment','$type','$positioncode',
                                            '$salary','$officecode','$from','$to','$date_created','$net_half_db','$tax_db')");
        ...
        if($numrow!=0) {
            echo '<script>swal("Duplication!", "Some employee already added on this payroll!", "error");</script>'; 
...
.then((willDelete) => {
  if (willDelete) {
document.location="view-payroll?delete='.$id.'&payroll_id='.$payroll_id.'";
  } else {
    swal("Delete Cancelled");
      document.location="view-payroll?payroll_id='.$payroll_id.'";
  }
   ...
   $query = mysqli_query($db, "DELETE FROM payroll_tbl WHERE name_id='$id' AND payroll_id='$payroll_id' ");
...
if(isset($_POST['update_deduction']) == 'update_deduction') {
    ...
    $query = mysqli_query($db, "SELECT salary_rate FROM payroll_tbl WHERE name_id='$name_id' AND payroll_id='$payroll_id' ");
                if($row=mysqli_fetch_array($query)){
    ...
    // }
    echo '<script>document.location="view-payroll?payroll_id='.$payroll_id.'"</script>';
   ...
   $charge = $_POST['edit_charge'];
    ...
    $query = mysqli_query($db, "UPDATE payroll_tbl SET charge='$charge'
                                WHERE payroll_id='$payroll_id' ");
...
<section class="section">
            ...
            foreach($officenames as $key=>$value) {
               ...
               if(++$i == $numItems) {
                ...
                echo $value.", ";
                    ...
                    $query = mysqli_query($db, "SELECT DISTINCT date_from,date_to,date_created,charge FROM payroll_tbl WHERE payroll_id = '$p_id' ");
                    if($row = mysqli_fetch_array($query)) {
                ...
                <table class="table table-striped" id="table1">
                    <thead>
                        <tr>
                            ...
                            <th style="font-size:12px!important;">Semimonthly Rate</th>
  ...
  $run_query = mysqli_query($db, "  SELECT payroll_tbl.name_id,payroll_tbl.payroll_id, payroll_tbl.salary_rate
    ,payroll_tbl.late_per_hr,payroll_tbl.absent,payroll_tbl.net_pay,
    payroll_tbl.late_per_minute,payroll_tbl.date_to, employment_info.employeeno, employment_info.surname,employment_info.firstname,employment_info.middlename,employment_info.extension, position.position ,department.officename
  FROM payroll_tbl 
  ...
  ON department.officecode = payroll_tbl.officecode
    INNER JOIN position
  ON payroll_tbl.position_code = position.positioncode 
  WHERE payroll_tbl.payroll_id = '$p_id'
  ");
 ...
 <?php if($row['middlename'] == '') {
  ...
  <td><?php echo $row['position'] ?></td>
   ...
   <?php $semi_salary =  $row['salary_rate'] / 2 ; ?>
 <td><?php echo number_format((float)$semi_salary, 2, '.', ','); ?></td>
<?php
...
<td><?php
  if($row['absent'] > 0) {  
  echo "<b style='font-size:15px'>".$row['absent']."</b> (".number_format((float)$late_deduction, 2, '.', ',').")"; 
} else {
   echo ' '; 
} 
?>
...
<td><?php
  ...
  echo "<b style='font-size:15px'>".$row['late_per_minute']."</b> (".number_format((float)$late_deduction, 2, '.', ',').")"; 
} else {
   echo ' '; 
} 
?>
...
<div class="modal fade" id="modal-edit-charge" tabindex="-1" role="dialog"
            ...
            <form action="view-payroll" method="post">
...
<div class="modal fade" id="add" tabindex="-1" role="dialog"
            ...
            <?php 
            $payroll_id = $_GET['payroll_id'];
            $query = mysqli_query($db, "SELECT DISTINCT date_from,date_to,date_created,working_days,charge,mode_of_payment 
                                        FROM payroll_tbl 
                                        WHERE payroll_id='$payroll_id' ");
            if($row=mysqli_fetch_array($query)) {
                $from = $row['date_from'];
            ...
            <input type="hidden" value="<?php echo $working_days; ?>" name="days">
...
<script>
  $(document).ready(function() {
    $('#table1').DataTable();
        ...
        'ajax':{
          'url':'load-view-payroll.php?payroll_id='+payroll_id,
          'type':'post',
